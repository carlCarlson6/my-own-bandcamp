...
export const getFavoriteAlbumsQuery = protectedProcedure
  .input(
    z.object({ page: z.number().int().min(1).default(1) }).default({})
  )
  .query(
    ({ ctx: { db, userId }, input: { page } }) => getFavoriteAlbums(db, userId, page)
  );

export const getFavoriteAlbums = async (db: Db, userId: string, page: number) => {
